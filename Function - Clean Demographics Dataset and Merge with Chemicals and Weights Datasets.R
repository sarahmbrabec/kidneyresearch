#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#~~~~~~~~  FUNCTION - CLEAN MERGED DEMOGRAPHICS DATASET & MERGE WITH CHEMICALS AND wEIGHTS DATASETS ~~~~~~~~~~#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#

# Purpose: clean the merged demographic dataset to eliminate excessive variables and harmonize variables that 
#          refer to the same demographic factor but have different NHANES codenames. Then extract the weights
#          variables pertaining to the Interview and MEC examination to merge with the weights dataset. And 
#          finally, form the merged chemical and demographics dataset. 
...
#                                      and MEC examination. 
# Outputs: returns a list containing the clean demographics dataset, the merged chemicals and demographics
#          dataset, and the finished weights dataset
                                                                                     ...
                                                                                     , weights_unfinished_dataset)
{
  ...
  demographics_unclean_dataset[study_year_1, "INDHHIN2"] <- demographics_unclean_dataset[study_year_1, "INDHHINC"]
  demographics_unclean_dataset[study_year_2, "INDHHIN2"] <- demographics_unclean_dataset[study_year_2, "INDHHINC"]
  demographics_unclean_dataset[study_year_3, "INDHHIN2"] <- demographics_unclean_dataset[study_year_3, "INDHHINC"]
  demographics_unclean_dataset[study_year_4, "INDHHIN2"] <- demographics_unclean_dataset[study_year_4, "INDHHINC"]
  
  # INDFMIN2 and INDFMINC represent the annual family income, so harmonize
  demographics_unclean_dataset[study_year_1, "INDFMIN2"] <- demographics_unclean_dataset[study_year_1, "INDFMINC"]
  demographics_unclean_dataset[study_year_2, "INDFMIN2"] <- demographics_unclean_dataset[study_year_2, "INDFMINC"]
  demographics_unclean_dataset[study_year_3, "INDFMIN2"] <- demographics_unclean_dataset[study_year_3, "INDFMINC"]
  demographics_unclean_dataset[study_year_4, "INDFMIN2"] <- demographics_unclean_dataset[study_year_4, "INDFMINC"]
  ...
  chem_demo_dataset <- merge(chemicals_clean_dataset
